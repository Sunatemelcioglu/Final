...
<!-- ============================== NOTLAR ================================= -->
<!-- ======================================================================= -->
[^1]: Öğrenci Numarası, [Github Repo](https://github.com/Sunatemelcioglu)
19080245
...
library(skimr)
skimr::skim_without_charts(superbowl_data)
...
Yapılan ANOVA testi sonuçları incelendiğinde playoffs değişkeninin defensive_ranking değişkenine göre istatistiksel olarak anlamlı bir ilişkisi olduğunu göstermektedir. ANOVA analizinde, faktör değişken (defensive_ranking) ile yanıt değişkeni (playoffs) arasındaki varyansın farklılığının anlamlılığı test edilmiştir.ANOVA tablosunda, factor(superbowl_data$defensive_ranking) satırı faktör değişkenin etkisini temsil ederken, Residuals satırı hata varyansını temsil eder. Df sütunu, özgürlük derecelerini gösterir. Sum Sq sütunu, toplam kareler toplamını temsil eder. Mean Sq sütunu, ortalama kareler toplamını gösterir. F value sütunu, faktör etkisinin istatistiksel anlamlılığını ifade eden F istatistiğini verir. Pr(>F) sütunu, p değerini gösterir.

Sonuçlar, faktör değişken olan defensive_ranking'in playoffs değişkenine etkisi olduğunu ve bu etkinin istatistiksel olarak anlamlı olduğunu göstermektedir.

Pr(>F) değeri 0.001'e yakın olduğu için, bu ilişkinin tesadüfi olmadığı ve anlamlı olduğu sonucuna varabiliriz.

Basit doğrusal regresyon analizinde "Kazanılan maçlar (superbowl_data$wins) ve attığı basket sayısı (points_for) değişkeni üzerinde anlamlı bir etkiye sahiptir." sonucuna ulaşılmıştır. Bu, her bir ekstra kazanılan maç için ortalama olarak puanların 16.9037 artacağını gösterir. Katsayılar bölümünde, eğim (slope) katsayısı olan superbowl_data$wins için tahmin edilen değer 16.9037'dir.

Ayrıca, kesme noktası (intercept) katsayısı olan Intercept için tahmin edilen değer 215.3192'dir. Bu, kazanılan maç sayısı sıfır olduğunda beklenen ortalama puanın 215.3192 olduğunu gösterir. 

R-kare (R-squared) değeri 0.533 olarak hesaplanmıştır, yani modelin açıkladığı varyansın %53.3'ünü kazanılan maçlar değişkeni açıklamaktadır. Bu, modelin veri setindeki değişikliğin büyük bir kısmını açıkladığını göstermektedir.

Son olarak, F istatistiği 726 olarak hesaplanmış ve p-değeri < 2.2e-16 (çok düşük) olarak bulunmuştur. Bu da, kazanılan maçlar değişkeninin puanları açıklamada istatistiksel olarak anlamlı olduğunu göstermektedir.

Bu sonuçlar, kazanılan maçlar ve attığı puanlar arasında pozitif bir ilişki olduğunu ve kazanılan her maçın takımın puanlarını artırdığını göstermektedir.
...
Veri setlerimizden birisi normal dağılımdan gelirken diğeri gelmiyor. Bu durumda Spearman korelasyon testi yapmalıyız. Spearman korelasyonu, sıralı veriler arasındaki ilişkiyi değerlendirmek için kullanılır ve değişkenlerin dağılımının normal olmasını gerektirmez.

## Sütun grafiği çizimi
ggplot(data, aes(x = superbowl_data$Team, y = superbowl_data$Wins)) +
...
## Playoffa kalan takımların kazanma sayıları
library(ggplot2)
